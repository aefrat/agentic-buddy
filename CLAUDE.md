...
18. **Use existing skills for external services.** When a skill or MCP tool exists for a service (Jira, Gmail, Google Docs, Slack, GitLab, GitHub), always use it instead of raw API calls, curl commands, or custom scripts. Skills handle auth, pagination, and error handling correctly. Reference: `jira:jira-mcp-management` for Jira queries, `google:gws-*` for Google Workspace, `slack:*` for Slack. Only fall back to direct API calls if no skill covers the specific need.
