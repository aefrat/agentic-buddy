...
- **CY26Q2 Quarterly Connection** -- Workday Talent Cycle submissions done for 9/10 members (Matt Goldman pending). PDR Talent Calibration Jul 23 completed - check outcome. QC reports and development briefs at `agent_brain/projects/qc-agent/active/Q2-2026/` and `talent-architecture/active/`. Roni has QE-to-SE transition as primary development recommendation. 3 HiPo nominations: Juanje, Kanitha, Roni. **Workday manager evaluations 17 days overdue (deadline Jul 10).**
- **Hubert Stefanski transition day 27 (started Jul 1).** Transferring to PitCrew team (not leaving Red Hat). Jeff Ligon takes scrum master from Manuel Sandino (no formal announcement yet). Knowledge transfer plan overdue. AutoSD CloudFront migration timeline needed (Hubert is fork maintainer).
- **Kanitha Chim retention - active.** Exploring internal mobility (informed Jul 13). 1:1 same day produced Toolchain Architect trial role, TPO reduction, distribution offloading plan. Assessment: structural (role composition), not fundamental (environment). **Jamie sync done Jul 15 - no objections to role change.** Mentor candidates: Alexander Larsson, Ian McLeod. Jan Onderka critical for distribution - Meital Arki follow-up (meeting was Jul 20, outcome unknown - 7 days, follow up). Remaining: Ozan distribution offload discussion, absorb TPO personally. RoG migration architecture proposal (4-phase, 21 packages) created Jul 14 as first Toolchain Architect deliverable - needs Petr + Rachel input. **Mentor finalization due ~Jul 28.** Tracking at `talent-architecture/active/retention/kanitha-chim.md`.
...
- **Aman Vishwakarma onboarding day 7 Mon Jul 28 (started Jul 18).** Intern conversion to ATC (RHIVOS Toolchain). Christine Freitas (manager) back from PTO (ended Jul 21) - coordinate on SP focal and buddy. Aman needs badge and laptop. Also key for Kanitha distribution offloading.
...
- **Roderick Kieley** -- 57 days into onboarding (started Jun 1). 30-day checkpoint 27 days overdue. SPSE (L5) development profile ready at `talent-architecture/active/roderick.kieley/`. PITCREW-294 due Jul 28 (tomorrow). **Roderick PTO Jul 27-Aug 3** - cannot deliver before leave. Needs timeline discussion Aug 4.
- **RHAS QE deliverables draft v1.0 (25 days old).** Test Strategy + Release Criteria HTMLs uploaded to Drive, emailed. User review needed before sharing with broader team. OCP QE outreach not initiated. RH release terminology gap analysis (Jul 26) found 9 TP + 9 GA gaps beyond current criteria - see `agent_brain/projects/rh-release-terminology.md`.
