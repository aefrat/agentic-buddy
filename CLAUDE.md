...
- **CY26Q2 Quarterly Connection** -- Workday Talent Cycle submissions done for 9/10 members (Matt Goldman pending). PDR Talent Calibration Jul 23 completed - check outcome (7 days, still unknown). QC reports and development briefs at `agent_brain/projects/qc-agent/active/Q2-2026/` and `talent-architecture/active/`. Roni has QE-to-SE transition as primary development recommendation. 3 HiPo nominations: Juanje, Kanitha, Roni. **Workday manager evaluations 20 days overdue (deadline Jul 10).**
- **Hubert Stefanski transition day 30 (started Jul 1).** Transferring to PitCrew team (not leaving Red Hat). Jeff Ligon takes scrum master from Manuel Sandino (no formal announcement yet). Knowledge transfer plan overdue. AutoSD CloudFront migration timeline needed (Hubert is fork maintainer). Attended OSCI meeting Jul 30 (KT context present).
- **Kanitha Chim retention - active.** Exploring internal mobility (informed Jul 13). 1:1 same day produced Toolchain Architect trial role, TPO reduction, distribution offloading plan. Assessment: structural (role composition), not fundamental (environment). **Jamie sync done Jul 15 - no objections to role change.** Mentor candidates: Alexander Larsson, Ian McLeod. Jan Onderka critical for distribution - Meital Arki follow-up (meeting was Jul 20, outcome unknown - 10 days, follow up). Remaining: Ozan distribution offload discussion, absorb TPO personally. RoG migration architecture proposal (4-phase, 21 packages) created Jul 14 as first Toolchain Architect deliverable - needs Petr + Rachel input. **OSCI confirmed willingness to onboard RHIVOS (Jul 30 meeting) - Adam Samalik creating tracking ticket, follow up mid-Aug.** **Mentor finalization was due ~Jul 28 - check status.** Tracking at `talent-architecture/active/retention/kanitha-chim.md`.
...
- **Aman Vishwakarma onboarding day 9 Wed Jul 30 (started Jul 18).** Intern conversion to ATC (RHIVOS Toolchain). Christine Freitas (manager) back from PTO (ended Jul 21) - coordinate on SP focal and buddy. Aman needs badge and laptop. Also key for Kanitha distribution offloading.
...
- **Roderick Kieley** -- 60 days into onboarding (started Jun 1). 30-day checkpoint 30 days overdue. SPSE (L5) development profile ready at `talent-architecture/active/roderick.kieley/`. PITCREW-294 was due Jul 28 - Roderick on PTO, could not deliver. **Roderick PTO Jul 27-Aug 3.** Needs timeline discussion Aug 4.
- **RHIVOS 2.0 Retro processed (Jul 30).** 7 action items finalized. **Avi directly assigned AI blocker summary PoC by Dana (end of August).** Uses existing Slack MCP, no AIA/PIA needed. Also: follow up Petr KWF meeting (Aug 11 1:1, confirmed never held), STAG automation status. Monitoring: Whitney cross-functional review (end Aug), Juanje handoff docs + tech channel (end Q3), Kanitha/Gadi/Petr SKU/CDN doc (end Q3), Luigi landing page (end Q3). Project at `agent_brain/projects/rhivos-2.0-retro.md`, action items at `user/rhivos-retro-action-items.md`.
- **RHAS QE deliverables draft v1.0 (28 days old).** Test Strategy + Release Criteria HTMLs uploaded to Drive, emailed. User review needed before sharing with broader team. OCP QE outreach not initiated. RH release terminology gap analysis (Jul 26) found 9 TP + 9 GA gaps beyond current criteria - see `agent_brain/projects/rh-release-terminology.md`. **RHAS testing meeting rescheduled to Aug 3** (moved due to RH company meeting).
