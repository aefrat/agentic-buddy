...
- **CY26Q2 Quarterly Connection** — Workday Talent Cycle submissions done for 9/10 members (Matt Goldman pending). PDR Talent Calibration Jul 23. QC reports and development briefs at `agent_brain/projects/qc-agent/active/Q2-2026/` and `talent-architecture/active/`. Roni has QE-to-SE transition as primary development recommendation. 3 HiPo nominations: Juanje, Kanitha, Roni.
- **Hubert Stefanski transition day 22 (started Jul 1).** Transferring to PitCrew team (not leaving Red Hat). Jeff Ligon takes scrum master from Manuel Sandino (no formal announcement yet). Knowledge transfer plan overdue. AutoSD CloudFront migration timeline needed (Hubert is fork maintainer).
...
- **Aman Vishwakarma onboarding day 5 (started Jul 18).** Intern conversion to ATC (RHIVOS Toolchain). Christine Freitas (manager) back from PTO (ended Jul 21) - coordinate on SP focal and buddy. Aman needs badge and laptop. Also key for Kanitha distribution offloading.
...
- **Roderick Kieley** — 52 days into onboarding (started Jun 1). 30-day checkpoint 22 days overdue. SPSE (L5) development profile ready at `talent-architecture/active/roderick.kieley/`. PITCREW-294 due Jul 28 (6 days), Roderick PTO Jul 27-Aug 3 - needs wrap-up before leave.
