...
- **CY26Q2 Quarterly Connection** — Workday Talent Cycle submissions done for 9/10 members (Matt Goldman pending). PDR Talent Calibration Jul 23 (today - check outcome next session). QC reports and development briefs at `agent_brain/projects/qc-agent/active/Q2-2026/` and `talent-architecture/active/`. Roni has QE-to-SE transition as primary development recommendation. 3 HiPo nominations: Juanje, Kanitha, Roni.
- **Hubert Stefanski transition day 23 (started Jul 1).** Transferring to PitCrew team (not leaving Red Hat). Jeff Ligon takes scrum master from Manuel Sandino (no formal announcement yet). Knowledge transfer plan overdue. AutoSD CloudFront migration timeline needed (Hubert is fork maintainer).
- **Kanitha Chim retention - active.** Exploring internal mobility (informed Jul 13). 1:1 same day produced Toolchain Architect trial role, TPO reduction, distribution offloading plan. Assessment: structural (role composition), not fundamental (environment). **Jamie sync done Jul 15 - no objections to role change.** Mentor candidates: Alexander Larsson, Ian McLeod. Jan Onderka critical for distribution - Meital Arki follow-up (meeting was Jul 20, check outcome). Remaining: Ozan distribution offload discussion, absorb TPO personally. Christine Freitas back from PTO (ended Jul 21) - connect this week. RoG migration architecture proposal (4-phase, 21 packages) created Jul 14 as first Toolchain Architect deliverable - needs Petr + Rachel input. Mentor finalization due ~Jul 28. Tracking at `talent-architecture/active/retention/kanitha-chim.md`.
...
- **Aman Vishwakarma onboarding day 6 (started Jul 18).** Intern conversion to ATC (RHIVOS Toolchain). Christine Freitas (manager) back from PTO (ended Jul 21) - coordinate on SP focal and buddy. Aman needs badge and laptop. Also key for Kanitha distribution offloading.
- **Talent Cycle July 2026 opened.** Engineering Competencies retired for SE/QE/EM/PM-Tech. New High Potential Flag (top 15%) replaces Future Talent. Skill Progression Matrices are the new framework. **9/10 Workday submissions done.** PDR Talent Calibration Jul 23 (7-9:05am CDT, Jaime). Distribution: 30% High Impact / 60% Successful / 10% Evolving (pending Matt). Action plan at `talent-architecture/active/talent-cycle-2026H2/`.
- **Roderick Kieley** — 53 days into onboarding (started Jun 1). 30-day checkpoint 23 days overdue. SPSE (L5) development profile ready at `talent-architecture/active/roderick.kieley/`. PITCREW-294 due Jul 28 (5 days), Roderick PTO Jul 27-Aug 3 - needs wrap-up before leave.
...
- [Talent Architecture](agent_brain/projects/talent-architecture/index.md) — Red Hat talent framework (Job Leveling, IC/EM progression, competency proficiency). Promoted back to level 4 on 2026-07-22 after access in 4 sessions since demotion (Jul 13, 19, 20, 21).
