...
- **RHIVOS developer-VM rebuild: path identified (Sep 1).** ProdSec blocker resolved in principle - Joe confirmed qcow images can be built in Brew today (Tomas Kopecek pulled in for build requirements). Approved distribution path: brew/konflux -> cdn -> customer portal (S3-built images not approved). Open decision: Brew (faster, one manual CDN-push step) vs Konflux (fully automated, ~1 sprint per Juanje) - time-driven. Artifact is the RHIVOS tooling/developer-VM. VROOM-52268 (build) / RHELDST-44190 (distribution).
