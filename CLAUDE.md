...
- **Hubert transition to RHAS approved** (Jeff Ligon & Paul Wallrabe, Jun 18). Next: discuss timelines with Hubert, communicate plan to stakeholders. Knowledge transfer plan overdue (~Jun 17).
...
20. **Slack: use bash curl with ~/.bashrc tokens, never MCP.** For all Slack API access, use `curl` with the `$SLACK_XOXC_TOKEN` and `$SLACK_XOXD_COOKIE` environment variables from `~/.bashrc`. Never use the Slack MCP plugin — it requires interactive OAuth re-auth and is unreliable. This applies to all skills, agents, and subagents that access Slack (scan-slack-channels, person-slack-lookup, manager report, etc.).
