...
- **2026-06-14:** PLM/ComposeDB additive-only import as a recurring release hazard — `importtree.py` never removes stale entries, and PLM auto-generates debuginfo listings from parent packages. Known issue in RHEL (8+ prior occurrences per RHELWF-11979), now hitting RHIVOS 2.0 because cross-compiler packages are new in the RHEL 10 base. Pattern: "when a shared, append-only registry is populated by successive builds, dropped entries persist as ghosts." Each RC compose will re-hit this unless Kanitha's proposed "delete-and-reimport" approach is adopted. Tracked in RHELWF-14266. (seen: 1)
