...
- **Ozan Unsal** — Working on OS build (create-OS-build process changes, container pulls from MR pipelines → Pulp upload). NXP smoke test (EBBR images) implemented but pipeline blocked on Pulp report upload issue.
...
| Ozan Unsal | Update smoke test ticket → done once pipeline unblocked | open |
| Hubert Stefanski | Validate S3+HTML fix on pre-prod, request review from Eitan | open |

### Key Themes

- Pipeline and build infrastructure: OS builds, smoke testing, Pulp upload issues
